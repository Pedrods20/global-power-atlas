...
- **Published benchmark inputs:** operator forecasts of demand, wind and solar for the delivery day are not part of this retrospective run, because the historical archive cannot say when each value became available. A prospective issue is not in that position: it records the instant it actually read the forecast, so it uses these features when the provider has already published the delivery day, and falls back to the information set above when it has not. Either way the issue's own manifest records which features it used. Realised delivery-day values are never substituted for a forecast.
