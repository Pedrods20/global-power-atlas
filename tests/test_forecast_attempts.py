...
def start(root, identifier="test-1", **kwargs):
    return attempts.start(root, identifier, zone="DE-LU", model="ridge",
                          delivery_date=DAY, started_at=STAMP, **kwargs)


def test_failed_attempt_remains_in_the_operational_denominator(tmp_path):
    start(tmp_path, origin="schedule")
    attempts.finish(tmp_path, "test-1", status="failed", error_type="UpstreamError", completed_at=STAMP)
...
def test_valid_issue_and_retry_keep_one_scheduled_day(tmp_path):
    ...
    attempts.finish(tmp_path, "test-1", status="issued", issue_id=first["issue_id"][0], completed_at=STAMP)
    start(tmp_path, "retry")
    later = record(tmp_path, stamp=STAMP.replace(hour=10))
    attempts.finish(tmp_path, "retry", status="issued", issue_id=later["issue_id"][0], completed_at=STAMP.replace(hour=10))
    row = attempts.report(tmp_path, start_date=DAY, end_date=DAY).row(0, named=True)
...
def test_attempt_paths_and_completion_clock_are_validated(tmp_path):
    ...
    with pytest.raises(ValueError, match="clock"):
        attempts.finish(tmp_path, "test-1", status="failed", completed_at=STAMP - dt.timedelta(seconds=1))
...
def test_cli_persists_failure_before_input_loading(tmp_path, monkeypatch):
    ...
    result = CliRunner().invoke(app, ["issue", "--delivery-date", str(DAY), "--output", str(tmp_path), "--attempt-id", "early-failure"])
...
def test_cli_persists_all_abstentions_and_nonzero_exit(tmp_path, monkeypatch):
    ...
    result = CliRunner().invoke(app, ["issue", "--delivery-date", str(DAY), "--output", str(tmp_path), "--attempt-id", "abstention"])
    assert result.exit_code == 1
...
def test_cli_attempt_lifecycle_and_missing_slot_report(tmp_path, monkeypatch):
    ...
    one = runner.invoke(app, ["forecast-attempt", "start", "--attempt-id", "workflow", "--delivery-date", str(DAY), *common])
    assert one.exit_code == 0, one.output
    two = runner.invoke(app, ["forecast-attempt", "finish", "--attempt-id", "workflow", "--status", "failed", "--if-open", *common])
    assert two.exit_code == 0, two.output
    report = runner.invoke(app, ["forecast-attempt", "report", "--start-date", str(DAY), "--end-date", str(DAY + dt.timedelta(days=1)), *common])
